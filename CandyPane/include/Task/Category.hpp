...
namespace candypane {
    ...
    class CategoryList; // forward declaration

    class Category {
    public:
        explicit Category(CategoryList* category_list = nullptr);

        void addTask(const Task& new_task);
        [[nodiscard]] std::string getName() const;
        [[nodiscard]] std::vector<Task> getTasks() const;
        unsigned long long getTasksSize();
        Task& getTaskById(unsigned long long int id);
        void insertTask(unsigned long long index, const Task& task);
        void relocateTaskById(unsigned long long int index, unsigned long long int category_id);
